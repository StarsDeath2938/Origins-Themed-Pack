...
**仅推荐使用 1.20.2 及以上的版本进行游玩。**_更低版本存在 更改出生位置导致游戏冻结_（在 1.20.2 修复）/ _玩家属性不会正确被修改_（在 1.20.2 的 1.12.0 版本中修复）_的问题。_

---

## 设计思路

> 本数据包是 _Salty Origins_ 的续作，你可以在我的 Git 仓库页面找到它（和它的美术资源）。  
>
> _和 Salty Origins 相同，这个数据包主要面向 **多人游戏** 。_ 但同时，我会在 _最大限度上_ 保证单人游戏时的游玩体验。

_为了适配多人对抗的游戏风格的同时保留鼓励各种族协作的玩法，尽最大可能贴合每个玩家的游玩风格_，我将 Origins添加的一部分种族进行了精简合并。
...
赋予玩家在水中呼吸的能力，以取代玩家在陆地上呼吸的能力。潮湿度将取代氧气条作为玩家剩余的可呼吸时间。
...
~~玩家将在距离初始重生点最近的 _暖海_ 出生。~~

玩家将在距离初始重生点最近的 _沙滩_ 出生。

部分版本依然存在的巨大性能消耗（很可能因为 `locate` 指令的同步执行），这个问题在 _1.20.2_ 版本的 _Origins Mod_ 得到改善。

但每次重生时，将重生点设置为较远的生物群系依然会导致 _2~5_ 秒的服务器延时。
...
接触水体或暴露在雨中会使玩家以每秒 **1** 点（♥）的速度失去生命值。
...
当玩家未接触水体或暴露在雨中时，按下 _首要能力按键_ （默认为 `G`）进入 _自燃_ 状态。

当玩家处于 _自燃_ 状态时：

持续为玩家赋予 _着火_ 状态。

缓慢恢复生命，其效果等同于 _生命恢复_ II。

持续获得伤害减免，其效果等同于 _抗性提升_ II。（ **40%** 全类型伤害减免）

获得 _缓慢_ I。

此效果将在玩家接触水体或暴露在雨中时，及再次按下 _首要能力按键_ 后立即结束。
...
玩家造成的近战攻击伤害将始终赋予 _着火_ 效果，其效果等同于 _火焰附加_ 附魔。
...
被 _雪球_ 攻击将使玩家受到 **1** 点（0.5 ♥）。

从 _细雪_ 带来的 _寒冷_ 效果中受到 **2** 倍伤害（2.5♥ / 2s）。
...
当玩家位于地狱或处于 _自燃_ 状态时，将会在周身环绕火焰粒子的效果。
